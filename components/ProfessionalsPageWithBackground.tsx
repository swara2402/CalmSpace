...
const EmergencyHelpSection: React.FC<{ helplines: Resource[] }> = ({ helplines }) => {
    ...
    return (
        <div className="p-4 bg-orange-100 dark:bg-orange-900 border-y-2 border-orange-400/50 dark:border-orange-500/50 animate-pulse-border">
            <div className="max-w-4xl mx-auto text-center">
                ...
                <p className="text-sm text-orange-700 dark:text-orange-300 mt-2">
                    If you or someone you know is in distress, please reach out to these 24/7 helplines.
                </p>
...
const ProfessionalCard: React.FC<{
    ...
    onBook: () => void;
    onViewReviews: () => void;
}> = ({ professional, onBook, onViewReviews }) => {
  return (
    <div className="bg-white/40 dark:bg-gray-800/40 backdrop-blur-md rounded-lg shadow-sm border border-white/20 dark:border-gray-700/50 p-4 flex flex-col sm:flex-row items-center sm:items-start gap-4 transition-all duration-300 hover:shadow-lg hover:border-purple-400/50">
      <img
        ...
        className="w-24 h-24 rounded-full flex-shrink-0 border-4 border-white/50 dark:border-gray-700/50"
      />
      <div className="flex-1 text-center sm:text-left">
        ...
        <button onClick={onViewReviews} className="flex items-center justify-center sm:justify-start gap-1 mb-3 group">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400 ml-1 group-hover:underline">
...
const BookingModal: React.FC<{ professional: Professional; onClose: () => void; }> = ({ professional, onClose }) => {
    ...
    const ProgressIndicator = () => {
        const steps: Step[] = ['date', 'type', 'payment', 'confirmed'];
                ...
                {steps.map((s, index) => (
                    <React.Fragment key={s}>
                        <div className="flex flex-col items-center">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors duration-300 ${index <= currentStepIndex ? 'bg-purple-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-500'}`}>
                                {index < currentStepIndex ? <i className="fas fa-check"></i> : index + 1}
                            </div>
                            <span className={`mt-2 text-xs capitalize ${index <= currentStepIndex ? 'text-gray-800 dark:text-gray-200 font-semibold' : 'text-gray-500'}`}>{s}</span>
                        </div>
                        {index < steps.length - 1 && <div className={`flex-1 h-1 mx-2 transition-colors duration-300 ${index < currentStepIndex ? 'bg-purple-600' : 'bg-gray-200 dark:bg-gray-700'}`}></div>}
                    </React.Fragment>
                ))}
            </div>
        );
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="w-full max-w-md bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-2xl shadow-2xl p-6 border border-white/20 dark:border-gray-700/50">
                ...
                {step !== 'confirmed' && <ProgressIndicator />}
                {renderStep()}
            </div>
        </div>
    )
...
const ProfessionalsPage: React.FC<{ user: User }> = ({ user }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        try {
            ...
            if (immediateHelp) {
                setHelplines(immediateHelp.resources.slice(0, 2));
            }
        } catch (err) {
  ...
  return (
    <div
      className="flex flex-col h-full max-w-6xl mx-auto bg-white/40 dark:bg-gray-800/40 backdrop-blur-lg rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 overflow-hidden relative"
      style={{
    ...
    >
      ...
      <div className="absolute inset-0 bg-black/30 dark:bg-black/50 backdrop-blur-[2px] z-0"></div>

      {/* Background decorative elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        {/* Professional-themed decorative icons */}
        <div className="absolute top-10 left-10 w-16 h-16 text-purple-300/20 animate-float">
          <i className="fas fa-user-md text-4xl"></i>
        </div>
        <div className="absolute top-32 right-20 w-12 h-12 text-blue-300/20 animate-float animation-delay-1000">
          <i className="fas fa-stethoscope text-3xl"></i>
        </div>
        <div className="absolute bottom-40 left-20 w-14 h-14 text-green-300/20 animate-float animation-delay-2000">
          <i className="fas fa-brain text-3xl"></i>
        </div>
        <div className="absolute bottom-20 right-10 w-18 h-18 text-purple-300/20 animate-float animation-delay-500">
          <i className="fas fa-heartbeat text-4xl"></i>
        </div>

        {/* Subtle gradient background shapes */}
        <div className="absolute -top-20 -left-20 w-40 h-40 bg-gradient-to-br from-purple-400/10 to-blue-400/10 rounded-full blur-3xl"></div>
        <div className="absolute -bottom-20 -right-20 w-40 h-40 bg-gradient-to-br from-green-400/10 to-purple-400/10 rounded-full blur-3xl"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-60 h-60 bg-gradient-to-br from-blue-400/5 to-purple-400/5 rounded-full blur-3xl"></div>

        {/* Additional wellness-themed elements */}
        <div className="absolute top-1/4 left-1/3 w-8 h-8 text-pink-300/15 animate-pulse">
          <i className="fas fa-spa text-2xl"></i>
        </div>
        <div className="absolute bottom-1/3 right-1/3 w-10 h-10 text-indigo-300/15 animate-pulse animation-delay-1500">
          <i className="fas fa-leaf text-2xl"></i>
        </div>
      </div>

      {viewingReviewsFor && <div className="relative z-20"><ReviewsModal professional={viewingReviewsFor} onClose={() => setViewingReviewsFor(null)} /></div>}
      {bookingWith && <div className="relative z-20"><BookingModal professional={bookingWith} onClose={() => setBookingWith(null)} /></div>}

      <header className="p-4 border-b border-white/20 dark:border-gray-700/50 text-center relative z-10">
      ...
      <div className="relative z-10"><EmergencyHelpSection helplines={helplines} /></div>
