...
const cryptoService = {
  ...
  deriveKey: async (pin: string, salt: Uint8Array): Promise<CryptoKey> => {
    ...
    return window.crypto.subtle.deriveKey(
      {
        name: 'PBKDF2',
        salt: salt as unknown as ArrayBuffer, // Type assertion to fix compatibility
  ...
  encrypt: async (data: string, key: CryptoKey): Promise<EncryptedPayload> => {
    ...
    return {
      iv: bufferToHex(iv.buffer as ArrayBuffer), // Convert to ArrayBuffer
      cipherText: bufferToHex(encryptedData as ArrayBuffer),
    };
  },
  ...
  decrypt: async (payload: EncryptedPayload, key: CryptoKey): Promise<string> => {
    try {
      ...
      const cipherBuffer = hexToBuffer(payload.cipherText);
